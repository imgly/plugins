 ...
 */
function rateLimitMiddleware<I, O extends Output>(options: RateLimitOptions) {
  const {
    maxRequests,
    timeWindowMs,
    keyFn = () => 'global',
  ...
  const middleware: Middleware<I, O> = async (input, options, next) => {
    ...
    if (tracker.timestamps.length >= maxRequests) {
      ...
      const remainingTimeMs = Math.max(0, timeWindowMs - (now - oldestTimestamp));
      ...
      if (onRateLimitExceeded) {
        ...
        const shouldProceed = await onRateLimitExceeded(input, options, rateLimitInfo);
        if (!shouldProceed) {
