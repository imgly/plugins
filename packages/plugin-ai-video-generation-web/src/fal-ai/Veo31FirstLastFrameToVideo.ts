...
export function Veo31FirstLastFrameToVideo(
  config: ProviderConfiguration
): (context: {
  cesdk: CreativeEditorSDK;
}) => Promise<Provider<'video', Veo31FirstLastFrameToVideoInput, VideoOutput>> {
  return async ({ cesdk }: { cesdk: CreativeEditorSDK }) => {
    const modelKey = 'fal-ai/veo3.1/first-last-frame-to-video';
  ...
  > = {
    ...
    input: {
      ...
      panel: {
        ...
        // @ts-ignore
        document: schema,
        inputReference: '#/components/schemas/Veo31FirstLastFrameToVideoInput',
        ...
        getBlockInput: async (input) => {
          ...
          // Parse duration from '8s' or '16s' format to number
