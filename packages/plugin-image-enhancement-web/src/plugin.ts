...
import {
  ...
  type Provider
} from './processImageEnhancement';
import { UserInterfaceConfiguration } from './types';

export const PLUGIN_ID = '@imgly/plugin-image-enhancement-web';
  ...
  return {
    initialize({ cesdk }) {
        ...
        process: (blockId, metadata) => {
          processImageEnhancement(
            ...
            pluginConfiguration.provider ?? {
              ...
              proxyURL: 'https://imgly-proxy.vercel.app/api/proxy/eyeq'
            }
          );
        }
      });
      ...
      const { translationsKeys } = registerFillProcessingComponents(cesdk, {
        pluginId: PLUGIN_ID,
        icon: '@imgly/plugin/image-enhancement',
        // @ts-ignore
      ...
      cesdk.setTranslations({
        en: {
          // @ts-ignore
          [translationsKeys.inspectorBarLabel]: 'Enhance Image',
          // @ts-ignore
          [translationsKeys.navigationBarLabel]: 'Enhance Image',
          // @ts-ignore
          [translationsKeys.canvasBarLabel]: 'Enhance Image',
          // @ts-ignore
          [translationsKeys.canvasMenuLabel]: 'Enhance Image',
          // @ts-ignore
          [translationsKeys.dockLabel]: 'Enhance Image'
        }
      });
    }
  };
...
function addIconSet(cesdk: CreativeEditorSDK) {
  cesdk.ui.addIconSet(
    '@imgly/plugin/image-enhancement',
    `
        <svg>
          <symbol
            fill="none"
            ...
            viewBox="0 0 24 24"
            id="@imgly/plugin/image-enhancement"
          >
