...
export function setPluginMetadata(
  ...
  engine.block.setMetadata(id, manifest.id, JSON.stringify(metadata));
}
...
export function getPluginMetadata(
  ...
  if (engine.block.hasMetadata(id, manifest.id)) {
    return JSON.parse(engine.block.getMetadata(id, manifest.id));
  } else {
    return {
      status: 'IDLE'
    };
...
export function clearPluginMetadata(engine: CreativeEngine, id: number) {
  if (engine.block.hasMetadata(id, manifest.id)) {
    engine.block.removeMetadata(id, manifest.id);
  }
}
