...
export class Commands<K extends string = string> extends Subscribable<CommandEvents, string> {
  ...
  async executeCommand<P = any, R = any>(cmd: K, params: P): Promise<R | undefined> {
    ...
    if (command) {
      this.#ctx.ui?.showNotification({ message: `${this.#ctx.i18n.translate(cmd)}`, type: "info" })
      try {
        return await command(this.#ctx, params);
      } catch (_e) {
        const e = _e as Error
        this.#ctx.ui?.showNotification({ message: `${this.#ctx.i18n.translate(cmd)} failed with ${e.message}`, type: "error" })
      }
    } else {
      this.#ctx.ui?.showNotification({ message: `${this.#ctx.i18n.translate(cmd)} not found!`, type: "warning" })
    }
  }
}
