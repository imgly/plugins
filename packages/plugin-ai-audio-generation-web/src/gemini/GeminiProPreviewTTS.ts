...
import schema from './GeminiProPreviewTTS.json';
import {
  truncate,
  createThumbnailFromAudio,
  getAudioDuration
} from '../elevenlabs/utils';
// Reuse the same voice list as Flash Preview
...
export interface ProviderConfiguration
  extends CommonProviderConfiguration<GeminiTTSInput, AudioOutput> {
  apiKey?: string;
  /**
...
export function GeminiProPreviewTTS(config: ProviderConfiguration) {
...
function getProvider(
  ...
  const modelKey = 'gemini/2.5-pro-preview-tts';
  ...
  const voiceSelectionPanelId = `${prefix}.voiceSelection`; // reuse same prefix
  const voiceAssetSourceId = createVoicesAssetSource(cesdk, baseURL);

  cesdk.setTranslations({
    en: {
      [`panel.${modelKey}`]: 'AI Voice (Gemini Pro)',
