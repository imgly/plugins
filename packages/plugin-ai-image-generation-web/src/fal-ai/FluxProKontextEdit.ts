...
function getProvider(
  ...
  const quickActions = createFluxProKontextEditQuickActions(cesdk);
...
function createFluxProKontextEditQuickActions(
  cesdk: CreativeEditorSDK
): QuickAction<FluxProKontextEditInput, ImageOutput>[] {
  ...
  return [
    QuickActionSwapImageBackground<FluxProKontextEditInput, ImageOutput>({
      mapInput: (input) => ({ ...input, image_url: input.uri }),
      cesdk
    }),
    QuickActionChangeImage<FluxProKontextEditInput, ImageOutput>({
      mapInput: (input) => ({ ...input, image_url: input.uri }),
      cesdk
    }),
    QuickActionImageVariant<FluxProKontextEditInput, ImageOutput>({
    ...
    QuickActionBaseSelect<FluxProKontextEditInput, ImageOutput>({
      quickAction: {
        id: 'styleTransfer',
        version: '1',
