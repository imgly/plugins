import { Runware, RunwareClient as SDKRunwareClient } from '@runware/sdk-js';

export type RunwareClient = InstanceType<typeof SDKRunwareClient>;

export interface RunwareProviderConfiguration {
  /**
   * WebSocket endpoint URL for the Runware proxy. The proxy handles API key injection.
   */
  proxyUrl: string;
  debug?: boolean;
  middlewares?: any[];
...
export function getImageDimensionsFromAspectRatio(
  aspectRatio: string
): { width: number; height: number } {
  return ASPECT_RATIO_MAP[aspectRatio] ?? { width: 1024, height: 1024 };
}

export function getImageDimensionsFromSize(
  imageSize: string
): { width: number; height: number } {
