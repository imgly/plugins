...
import schema from './QwenImage.json';
...
export function QwenImage(config: RunwareProviderConfiguration) {
  return async ({ cesdk }: { cesdk: CreativeEditorSDK }) => {
    addIconSetOnce(cesdk, '@imgly/plugin/formats', Icons.Formats);

    return createImageProvider<QwenImageInput>(
      {
        modelAIR: 'runware:108@1',
        providerId: 'runware/runware/qwen-image',
        name: 'Qwen-Image',
        // @ts-ignore
        schema,
        inputReference: '#/components/schemas/QwenImageInput',
        cesdk,
